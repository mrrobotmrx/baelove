...
$app->post('/feed','feed');
$app->post('/myfeed','myfeed'); /* User Feeds  */
...
function login() {
    ...
    try {
        ...
        $db = getDB();
        $userData ='';
        $sql = "SELECT user_id, name, email, username, foto FROM users WHERE (username=:username or email=:username) and password=:password ";
        $stmt = $db->prepare($sql);
...
function signup() {
    ...
    $email=$data->email;
    $name=$data->name;
    $foto = "http://www.ecpgr.cgiar.org/fileadmin/templates/ecpgr.org/images/empty-avatar.jpg";
    ...
    try {
        ...
        if (strlen(trim($username))>0 && strlen(trim($password))>0 && strlen(trim($email))>0 && $email_check>0 && $username_check>0 && $password_check>0)
        {
            ...
            if($mainCount==0)
            {
                ...
                $sql1="INSERT INTO users(username,password,email,name,foto)VALUES(:username,:password,:email,:name, :foto)";
                ...
                $stmt1->bindParam("foto", $foto,PDO::PARAM_STR);
                $stmt1->execute();
...
function feed(){
    ...
    try {
        ...
        if($systemToken == $token){
            ...
            $db = getDB();
            if($lastCreated){
                $sql = "SELECT * FROM feed WHERE created < :lastCreated ORDER BY feed_id DESC LIMIT 10";
                $stmt = $db->prepare($sql);
                
                $stmt->bindParam("lastCreated", $lastCreated, PDO::PARAM_STR);
            }
            else{
                $sql = "SELECT * FROM feed ORDER BY feed_id DESC LIMIT 10";
                $stmt = $db->prepare($sql);
